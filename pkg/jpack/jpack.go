...
func Get(data []byte, keys ...string) (value []byte, dataType int, soff int, offset int, err error) {
	...
	if len(keys) > 0 {
		for ki, k := range keys {
			//fmt.Printf(" dbgb0 => key  = [%v] offset %d  ki %d, \n", k, offset, ki) // string(data[:20]))
			lk := len(k)
			...
			for true {
				...
				//fmt.Printf(" ##1 k [%s] idx : %v  data [%s] \n offset %d \n data #2 [%v]\n",k, idx, string(data[:2]), offset, data[:2] )

				if idx := bytes.Index(data[offset:], []byte(k)); idx != -1 && (ln-(offset+idx+lk+2)) > 0 {
					//fmt.Printf(" ##2 k [%s] idx : %v \n",k, idx )
					offset += idx
					...
					return []byte{}, NotExist, -1,  -1, errors.New("Key xxx path not found")
				}
			}
		}
	} else {
